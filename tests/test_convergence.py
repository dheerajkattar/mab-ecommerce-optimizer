...
def _probe_best_pct(
    ...
    count = sum(
        1 for _ in range(n_probes) if strategy.select_arm(experiment_id, arm_ids) == best_arm
    )
...
class TestConvergence(unittest.TestCase):
    ...
    def test_compare_convergence_speed_across_all_samples(self) -> None:
        ...
        # Print report
        print(f"\n{'=' * 80}")
        print(f"Convergence Speed Comparison — {N_SAMPLES} samples, {N_ROUNDS} rounds each, 4 arms")
        print(f"{'=' * 80}")
        header = (
            f"{'Strategy':<30} {'Med. Conv':>10} {'Mean Regret':>12} {'Converged':>10} {'Wins':>6}"
        )
        print(header)
        print("-" * 70)
        for name, med_conv, mean_reg, n_conv, wins in summary:
            ...
            print(
                f"{name:<30} {med_str:>10} {mean_reg:>12.2f} {n_conv:>6}/{N_SAMPLES:<3} {wins:>6}"
            )
        print("-" * 70)
        print(
            f"Best overall: {summary[0][0]} "
